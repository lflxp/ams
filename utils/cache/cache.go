...
func init() {
	beego.Debug("初始化缓存实例")
	defaultExp,_ := time.ParseDuration("24h")
	gcInterval,_ := time.ParseDuration("3s")
	Cached = NewCache(defaultExp,gcInterval)
}
...
//设置数据项，无锁操作
func (c *Cache) SetNoLock(k string, v interface{}, d time.Duration) {
	var e int64
	if d == defaultExpiration {
		d = c.defaultExpiration
	}
...
//获取数据项，并判断数据项是否过期
func (c *Cache) GetNoLock(k string) (interface{}, bool) {
